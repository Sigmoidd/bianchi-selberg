...
\label{eq:C1order}
  ...
  \Bigr),
\end{equation}
with no hidden $N\mathfrak{p}$ or gluing dependence.  (With the sharp
$C_K=1$ there is no $e^{\pi r/2}$ factor in $C_1$; the classical looser
$C_K^{\mathrm{cl}}$ reintroduces it only inside $A_{\mathrm{cusp}}$.)
\end{theorem}
...
\label{eq:step1}
  ...
  \tag{1}
\end{equation}
where the factor $(1+\sqrt{\lambda})$ inside $A_{\mathrm{bdry}}$ is reserved
for the $H^2$/elliptic control of Step~2 (it majorizes the passage from
$H^1$ to the graph norm of $\Delta-\lambda$ on the compact core).
...
\label{eq:step2d}
  ...
  \tag{2d}
\end{equation}
using $h_{\min}=0.25$.  Combining~\eqref{eq:step2c},~\eqref{eq:step2d},
Lemma~\ref{lem:metric}, and $\norm{\Delta e}_{L^2_{\mathrm{hyp}}}
\le A_{\mathrm{met}}\,C_{\Delta}\,\norm{e}_{H^2_{\mathrm{euc}}}$, we obtain
\begin{equation}
\label{eq:step2e}
  \norm{(\Delta-\lambda)e_{\mathrm{bdry}}}_{L^2_{\mathrm{hyp}}}
  \le
  ...
  \delta_{\mathrm{aut}}
  /
  \bigl(C_{\mathrm{trace}}C_{\mathrm{Sob}}A_{\mathrm{met}}(1+\sqrt{\lambda})\bigr)
  \cdot
  A_{\mathrm{bdry}}
  \le
  A_{\mathrm{res}}\,A_{\mathrm{bdry}}\,\delta_{\mathrm{aut}},
  \tag{2e}
\end{equation}
after inserting $A_{\mathrm{ell}}=1+\lambda\ge C_{\Delta}(1+\lambda^{1/2})$ as a
convenient majorant of the elliptic/scaling constants on $K_Y$ (the factor
$1+\lambda$ absorbs both $\Delta e$ and $\lambda e$).  Therefore
\begin{equation}
\label{eq:step2}
  \norm{(\Delta-\lambda)u}_{L^2_{\mathrm{hyp}}(K_Y)}
  \le
